 ...
 * 
 * @challenge The big challenge with the current shortcuts implementation is, that every shortcut fires
 *            multiple times.
 *            Be sure to handle this appropriately. See Votevisualiser.prepareVisualisation() for Tills
 *            solution.
 * 
 */
public class EventHandling implements NativePreviewHandler, ClickHandler {
	...
	private void testLoadAction() {
		// implement test load (use QueueManager to add a large amount of strings to the queue)
		...
		for (int i = 0; i < counter; i++) {
			...
			env.getQueueManager().addNewSearchTerm("Microsoft " + i, startDate, endDate, callback);
		}
	}
